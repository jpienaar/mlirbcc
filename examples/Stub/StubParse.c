...
MlirBytecodeStatus mlirBytecodeOperationStateAddAttributeDictionary(
    void *context, MlirBytecodeOperationStateHandle state, MlirBytecodeAttrHandle attr) {
  return mlirBytecodeSuccess();
}
...
MlirBytecodeStatus mlirBytecodeOperationStatePop(
...
bool mlirBytecodeOperationWasLazyLoaded(
...
MlirBytecodeStatus mlirBytecodeOperationBlockPush(
    void *context, MlirBytecodeOperationHandle op, MlirBytecodeSize numArgs) {
  return mlirBytecodeSuccess();
}
...
MlirBytecodeStatus mlirBytecodeAssociateAttributeRange(
    void *context, MlirBytecodeAttrHandle handle, MlirBytecodeDialectHandle dialect,
...
MlirBytecodeStatus mlirBytecodeAssociateTypeRange(
    void *context, MlirBytecodeTypeHandle handle, MlirBytecodeDialectHandle dialect,
    MlirBytecodeBytesRef bytes, bool hasCustom) {
  return mlirBytecodeSuccess();
}
...
MlirBytecodeStatus mlirBytecodeDialectCallBack(
    void *context, MlirBytecodeDialectHandle dialect, MlirBytecodeStringHandle name) {
  return mlirBytecodeSuccess();
}
...
MlirBytecodeStatus mlirBytecodeDialectVersionCallBack(
    void *context, MlirBytecodeDialectHandle dialect, MlirBytecodeBytesRef version) {
  return mlirBytecodeSuccess();
}
...
MlirBytecodeStatus mlirBytecodeResourceDialectGroupEnter(
    void *context, MlirBytecodeDialectHandle dialect,
    MlirBytecodeSize numResources) {
  return mlirBytecodeSuccess();
}

MlirBytecodeStatus mlirBytecodeResourceExternalGroupEnter(
...
MlirBytecodeStatus mlirBytecodeAssociateStringRange(
    void *context, MlirBytecodeStringHandle handle, MlirBytecodeBytesRef bytes) {
  return mlirBytecodeSuccess();
}
...
MlirBytecodeStatus mlirBytecodeGetStringSectionValue(
...
int main(int argc, char **argv) {
  ...
  if (fstat(fd, &fileInfo) == -1) { close(fd); return 1; }
  uint8_t *stream = (uint8_t *)mmap(0, fileInfo.st_size, PROT_READ, MAP_SHARED, fd, 0);
  if (stream == MAP_FAILED) { close(fd); return 1; }
